(function () {
  ...
  const DEFAULT_TASK_PANEL_HINT = "这里统一处理状态、进度和备注。";
  const REPEAT_LABELS = {
    none: "不重复",
    daily: "每日",
    weekly: "每周",
  ...
  function handleTaskContainerChange(event) {
    ...
    if (prioritySelect) {
      const result = mutateTask(prioritySelect.dataset.taskId, function (task) {
        return sanitizeTask({
          ...task,
          priority: PRIORITY_VALUES.includes(prioritySelect.value) ? prioritySelect.value : task.priority,
          updatedAt: new Date().toISOString(),
        });
      });
      if (result) {
        renderAll();
        showNotice("任务优先级已更新。", "success");
      }
      return;
    }
  ...
  function renderTodayTaskList() {
    const today = toISODate(new Date());
    const allTodayTasks = sortTodayTasksOverview(getTasksForDate(today));
  ...
  function buildTodayTaskItem(task, isHandled) {
    ...
    return (
      ...
      "</span>" +
      '<span class="priority-chip today-priority-pill priority-' +
    ...
    return tasks
      .map(function (task) {
        ...
        return (
          ...
          "</select></label>" +
          '<label class="task-priority-inline" aria-label="调整 ' +
          escapeHtml(task.title) +
          ' 的优先级"><span>优先级</span><select class="subtle-select priority-inline-select" data-priority-select="true" data-task-id="' +
          task.id +
          '" aria-label="调整 ' +
          escapeHtml(task.title) +
          ' 的优先级">' +
  ...
  function sortTodayTasksOverview(tasks) {
    return tasks.slice().sort(compareTodayTaskPriority);
  }

  function compareTodayTaskPriority(left, right) {
    const leftHandledRank = Number(isTerminalStatus(left.status));
    const rightHandledRank = Number(isTerminalStatus(right.status));

    return (
      PRIORITY_ORDER[left.priority] - PRIORITY_ORDER[right.priority] ||
      leftHandledRank - rightHandledRank ||
      normalizedTimeForSort(left.reminderTime).localeCompare(normalizedTimeForSort(right.reminderTime)) ||
      left.createdAt.localeCompare(right.createdAt) ||
