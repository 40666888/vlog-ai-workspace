(function () {
  ...
  function bindEvents() {
    ...
    elements.cancelPreviewBtn.addEventListener("click", clearBatchPreview);
  ...
  function handleTaskContainerChange(event) {
    const progressSelect = event.target.closest("[data-progress-select]");
    if (progressSelect) {
      const result = mutateTask(progressSelect.dataset.taskId, function (task) {
        return applyProgressChange(task, Number(progressSelect.value));
      });
      if (result) {
        renderAll();
  ...
  function updateNotificationState() {
    if (!("Notification" in window)) {
      elements.notificationState.textContent = "当前浏览器不支持系统通知，但页面内时间线和统计仍可使用。";
    ...
    if (Notification.permission === "granted") {
      elements.notificationState.textContent = "浏览器提醒已开启。只要页面保持打开，到点会发出提醒通知。";
      elements.requestNotificationBtn.textContent = "提醒已开启";
      return;
    }

    if (Notification.permission === "denied") {
      elements.notificationState.textContent = "浏览器提醒已被禁用，你仍然可以通过时间线和日历查看提醒安排。";
      elements.requestNotificationBtn.textContent = "提醒已被禁用";
      return;
    }

    elements.notificationState.textContent = "浏览器提醒尚未开启。授权后，页面打开时会在任务到点时弹出通知。";
  ...
  function parseTaskLine(line, defaults) {
    try {
      ...
      const leadingTimeMatch = title.match(/^(\d{1,2}:\d{2})\s+(.+)$/);
      if (leadingTimeMatch) {
        const normalizedLeadingTime = normalizeTime(leadingTimeMatch[1]);
        if (!normalizedLeadingTime) {
          throw new Error("开头的提醒时间格式不正确。");
        }
      ...
      return {
        ok: false,
        error: error && error.message ? error.message : "这一行无法解析。",
      };
    }
  }
  ...
  function renderMetaStrip() {
    elements.saveStatus.textContent = state.meta.lastSavedAt
      ? "保存 " + formatDateTime(state.meta.lastSavedAt)
      : "尚未保存";
    elements.exportStatus.textContent = state.meta.lastExportedAt
      ? "导出 " + formatDateTime(state.meta.lastExportedAt)
      : "未导出";
  }
      ...
      ? errorRows
          .map(function (row) {
            return (
              ...
              row.lineNumber +
              " 行</span>" +
              '<span class="status-chip status-overdue">将跳过</span>' +
              "</div>" +
  ...
  function renderSidePanel() {
    Array.from(elements.sideTabs.querySelectorAll("[data-side-tab]")).forEach(function (button) {
      button.classList.toggle("is-active", button.dataset.sideTab === state.ui.sideTab);
    });
    elements.timelinePanel.classList.toggle("is-hidden", state.ui.sideTab !== "timeline");
    elements.pendingPanel.classList.toggle("is-hidden", state.ui.sideTab !== "pending");
  }
  ...
  function renderSummaryCards() {
    const selectedStats = createStats(getTasksForDate(state.ui.selectedDate), toISODate(new Date()));
    const periodStats = createPeriodStats();
    const cards = [
      {
        title: state.ui.selectedDate === toISODate(new Date()) ? "今天" : "选中日期",
        label: formatDisplayDate(state.ui.selectedDate),
        stats: selectedStats,
      },
      {
        title: "本周",
  ...
  function renderTodayTaskList() {
    ...
    elements.todayTaskCount.textContent = openTasks.length ? openTasks.length + " 项待推进" : allTodayTasks.length ? "已全部完成" : "0 项";
    ...
    if (!openTasks.length) {
      elements.todayTaskList.innerHTML = buildEmptyState("今天的任务已全部完成。", "如果还有临时事项，可以直接在上方快速录入。");
      return;
    }

    elements.todayTaskList.innerHTML = openTasks
      .map(function (task) {
        const quickAction = shouldOfferQuickComplete(task) ? "quick-complete" : "focus-task";
        const actionLabel = quickAction === "quick-complete" ? "完成" : "继续";
        return (
          '<article class="today-task-item" data-task-focus="' +
          task.id +
          '">' +
  ...
  function renderCalendar() {
    ...
    for (let index = 0; index < 42; index += 1) {
      ...
      const summaryClass = dayStats.overdue
        ? "is-warning"
  ...
  function renderTaskList() {
    ...
    elements.taskResultMeta.textContent = tasks.length + " 条结果";
    ...
    elements.taskList.innerHTML = renderTaskCards(tasks, { compact: false });
  }
  ...
  function renderTaskCards(tasks, options) {
    ...
    return tasks
      .map(function (task) {
        ...
        const reminderLabel = task.reminderTime || "未设提醒";

        if (compact) {
          return (
            '<article class="task-card is-compact" data-task-card="true" data-task-id="' +
            task.id +
            '">' +
            ...
            "</div>" +
            "</div>" +
            '<div class="task-progress-badge"><strong>' +
            task.progress +
            '%</strong><span>当前进度</span></div>' +
            "</div>" +
            ...
            '<button class="text-btn" type="button" data-action="mark-complete" data-task-id="' +
            task.id +
            '">标记完成</button>' +
            '<button class="text-btn" type="button" data-action="postpone" data-task-id="' +
            task.id +
            '">延期到明天</button>' +
        ...
        return (
          '<article class="task-card" data-task-card="true" data-task-id="' +
          task.id +
          '">' +
          '<div class="task-card-body">' +
          '<div class="task-head">' +
          ...
          "</div>" +
          '<div class="task-progress-block">' +
          '<div class="progress-head"><span>真实进度</span><strong data-progress-value="true" data-task-id="' +
          task.id +
          '">' +
          task.progress +
          "%</strong></div>" +
          ...
          '<label class="field"><span>当前状态</span><select data-status-select="true" data-task-id="' +
          task.id +
          '">' +
          ...
          task.id +
          '">' +
          '<select data-progress-select="true" data-task-id="' +
          task.id +
          '">' +
          buildOptionsHtml(QUICK_PROGRESS_VALUES, task.progress, function (value) {
            return "设为 " + value + "%";
          }) +
          "</select>" +
          "</div>" +
          ...
          task.id +
          '">延期到明天</button>' +
          '<button class="text-btn" type="button" data-action="toggle-edit" data-task-id="' +
          task.id +
          '">' +
          (runtime.editingTaskId === task.id ? "收起编辑" : "编辑") +
          "</button>" +
          '<button class="text-btn" type="button" data-action="delete" data-task-id="' +
          task.id +
          '">删除</button>' +
          '<button class="ghost-btn" type="button" data-action="jump-date" data-task-id="' +
          task.id +
  ...
  function renderStorageMeta() {
    elements.storageMeta.textContent =
      ...
      " · 任务总数：" +
      state.tasks.length +
      (state.meta.lastSavedAt ? " · 最近保存：" + formatDateTime(state.meta.lastSavedAt) : "") +
      (state.meta.lastExportedAt ? " · 最近导出：" + formatDateTime(state.meta.lastExportedAt) : "");
  }
  ...
  function createPeriodStats() {
    ...
    return {
      week: {
        ...
        ...createStats(getTasksInRange(toISODate(weekStart), toISODate(weekEnd)), today),
      },
      month: {
        label: anchor.getFullYear() + " 年 " + (anchor.getMonth() + 1) + " 月",
        ...createStats(getTasksInRange(toISODate(monthStart), toISODate(monthEnd)), today),
      },
      year: {
        label: anchor.getFullYear() + " 年",
        ...createStats(getTasksInRange(toISODate(yearStart), toISODate(yearEnd)), today),
      },
    };
  }
  ...
  function getNextRepeatDate(isoDate, repeat) {
    ...
    if (repeat === "monthly") {
      return toISODate(new Date(date.getFullYear(), date.getMonth() + 1, date.getDate()));
    }
    return isoDate;
  }
