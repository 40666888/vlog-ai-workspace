(function () {
  ...
  function renderTaskCards(tasks) {
    return tasks
      .map(function (task) {
        ...
        const progressValues = getProgressOptionValues(task.progress);
        ...
        return (
          ...
          task.progress +
          '%;"></span></div>' +
          '<label class="field compact-field task-status-field"><span>状态切换</span><select data-status-select="true" data-task-id="' +
          task.id +
          '">' +
          ...
          task.id +
          '">' +
          buildOptionsHtml(progressValues, task.progress, function (value) {
            return "设为 " + value + "%";
          }) +
          "</select>" +
          "</div>" +
          ...
          (!isTerminalStatus(task.status)
            ? '<button class="secondary-btn task-inline-btn" type="button" data-action="postpone" data-task-id="' +
              task.id +
              '">延期到明天</button>'
            : "") +
  ...
  function getProgressOptionValues(progress) {
    const value = clamp(Number(progress) || 0, 0, 100);
    if (QUICK_PROGRESS_VALUES.includes(value)) {
      return QUICK_PROGRESS_VALUES;
    }

    return QUICK_PROGRESS_VALUES.concat([value]).sort(function (left, right) {
      return left - right;
    });
  }
