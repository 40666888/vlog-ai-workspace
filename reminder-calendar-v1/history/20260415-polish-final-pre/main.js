(function () {
  ...
  function syncResponsiveFilterPanel() {
    ...
    if (!isCompact) {
      ...
      elements.filterMoreToggle.setAttribute("aria-expanded", "true");
  ...
  function renderStorageMeta() {
    elements.storageMeta.textContent =
      ...
      state.tasks.length +
      " 条任务" +
      (state.meta.lastSavedAt ? " · 保存 " + formatDateTime(state.meta.lastSavedAt) : "") +
      (state.meta.lastExportedAt ? " · 导出 " + formatDateTime(state.meta.lastExportedAt) : "");
  }
